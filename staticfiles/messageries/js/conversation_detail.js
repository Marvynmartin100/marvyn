...
/*
 // === Suppression des messages via AJAX ===
function setupDeleteMessageButtons() {
  const deleteButtons = document.querySelectorAll(".delete-message-btn");

  deleteButtons.forEach(btn => {
    btn.addEventListener("click", function() {
      const deleteUrl = this.dataset.deleteUrl; // URL correcte fournie dans le template
...
// Appeler cette fonction après le chargement des messages
document.addEventListener("DOMContentLoaded", () => {
  setupDeleteMessageButtons();
});


document.addEventListener("DOMContentLoaded", () => {
    // === Compteur de caractères pour l'envoi de message ===
    ...
    // === Envoi dynamique des messages via AJAX ===
    const form = document.getElementById("form-envoi-message");
    if (form) {
        form.addEventListener("submit", function(event) {
            event.preventDefault(); // Empêche le rechargement
            console.log("Submit intercepté"); // Vérifie que le submit est bien intercepté



            const contenu = input.value.trim();
            console.log("Contenu à envoyer:", contenu); // Vérifie le contenu saisi
            if (!contenu) return;
            console.log("URL fetch:", form.action);
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (messagesContainer) {
                        // Crée le div du nouveau message
                        const newMessage = document.createElement("div");
                        newMessage.classList.add("message", "sent"); // classes CSS existantes
                        newMessage.dataset.messageId = data.message.id;

                        // Insère uniquement le contenu et le statut
                        newMessage.innerHTML = `
                            <div class="message-content">${data.message.contenu}</div>
                            <div class="message-meta">
                                <time class="message-time" datetime="${data.message.date_envoi}">
                    ...
                    console.error(data.error || "Erreur lors de l'envoi du message.");
                }
            })
            .catch(err => {
